...
type Config struct {
	Level           string `envconfig:"optional"` // when true, use a more readable (non-JSON) format
	HumanFriendly   bool   `envconfig:"optional"` // disable ANSI colors in human-friendly output
	NoColoredOutput bool   `envconfig:"optional"` // log level name, e.g. "debug", "info", "warn", "error"
}
